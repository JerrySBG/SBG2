#!/bin/bash
# =========================================
UPDATE="https://raw.githubusercontent.com/JerrySBG/SBG2/main/update/update.sh"
GREEN='\033[0;32m'
RED='\033[0;31m'
...
checking_sc() {
  ...
  if [[ $date_list < $useexp ]]; then
    ...
    echo -e "   \033[0;33mTu IP de VPS${NC} $ipsaya \033[0;33mA Sido BANEADA${NC}"
    echo -e "     \033[0;33mCompra Acceso al Scripts${NC}"
...
if [ "$res" = "Expirado" ]; then
Exp="\e[36mExpirado\033[0m"
else
...
# // Root Checking
if [ "${EUID}" -ne 0 ]; then
                echo -e "${EROR} Por Favor, Iniciar Como Usuario ROOT !"
                exit 1
fi
...
echo ""
if [ -z $host ]; then
echo -e "  [INFO] Introduce Tu Dominio/Subdominio"
echo -e "$COLOR1└─────────────────────────────────────────────────┘${NC}"
echo ""
...
clear
echo -e "${GREEN}┌─────────────────────────────────────────────────────┐${NC}"
echo -e "  ${COLBG1}                - INFORMACION VPS -                  ${NC}   "
echo -e "${GREEN}└─────────────────────────────────────────────────────┘${NC}"
...
echo -e "${GREEN}┌─────────────────────────────────────────────────────┐${NC}"
echo -e "${CYAN}   Usuario ${NC}      =${LIGHT} $Name ${NC}"
echo -e "${CYAN}   Expiracion${NC}    =${RED} $dayleft Dias ${NC}"
echo -e "${GREEN}└─────────────────────────────────────────────────────┘${NC}"
echo ""
