...
mkdir /etc/xray
mkdir /etc/kyt
mkdir /etc/kyt/trojan
mkdir /etc/kyt/vless
mkdir /etc/kyt/vmess
mkdir /etc/kyt/limit
mkdir /etc/kyt/limit/trojan
mkdir /etc/kyt/limit/vless
mkdir /etc/kyt/limit/vmess
mkdir /etc/kyt/limit/ssh
mkdir /etc/kyt/limit/ssh/ip
mkdir /etc/kyt/limit/trojan/ip
mkdir /etc/kyt/limit/vless/ip
mkdir /etc/kyt/limit/vmess/ip
mkdir /etc/kyt/log
mkdir /etc/kyt/log/trojan
mkdir /etc/kyt/log/vless
mkdir /etc/kyt/log/vmess
mkdir /etc/kyt/log/ssh
mkdir /etc/vmess
mkdir /etc/vless
mkdir /etc/trojan
mkdir /etc/bot
...
printf "\e[1;92m     .::.\e[0m\e[1;77m Input Domain \e[1;92m.::.\e[0m\n"
echo -e "\033[1;93m──────────────────────────────────────\033[0m"
echo -e ""
...
izin_dulu_dong() {
  # Link izin IP VPS
  url_izin='https://raw.githubusercontent.com/JerrySBG/SBG2/main/izin.txt'
  ...
  if [[ -n "$izin" ]]; then
    while IFS= read -r line; do
      ...
      if [[ "$ipvps" == "$ip_vps" ]]; then
        echo "Nombre VPS: $nama"
        echo "IP VPS: $ipvps"
        echo "Fecha de Vencimiento: $tanggal"
        break
      fi
    done <<< "$izin"
...
clear
      ...
      echo -e "                 • JERRY VIP SBG •                 "
      echo -e "\e[33m ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\033[0m" | lolcat
      echo -e ""
      echo -e "\e[93m Nombre\e[32;1m   : $nama "
      echo -e "\e[93m IP VPS\e[32;1m : $ip_vps"
      echo -e "\e[93m Dominio\e[32;1m : $(cat /etc/xray/domain)"
      echo -e ""
      echo -e "\e[93m SSH\e[32;1m    : BLOQUEADO "
      echo -e "\e[93m VMESS\e[32;1m  : BLOQUEADO "
      echo -e "\e[93m VLESS\e[32;1m  : BLOQUEADO "
      echo -e "\e[93m TROJAN\e[32;1m : BLOQUEADO "
      echo -e ""        
      echo -e "${red} Tu VPS no tiene PERMISOS \e[32;1m "
      echo -e "${red} Póngase en contacto con el administrador para obtener SCRIPT \e[32;1m" | lolcat
      echo -e ""
      echo -e "\e[93m Telegram\e[32;1m : https://t.me/Jerry_SBG"
      echo -e "\e[33m ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\033[0m" | lolcat
      echo -e ""
      exit 0
    fi
  else
    echo "El contenido de PERMISOS no se ENCONTRO"
    exit 0
  fi
  clear
}
...
frontend ssh-ssl
    bind *:443 ssl crt /etc/haproxy/kyt.pem
    mode tcp
    option tcplog
...
wget -O /etc/nginx/nginx.conf https://raw.githubusercontent.com/JerrySBG/SBG2/main/nginx.conf
...
#<BR><font color='#008080'>Selamat Datang di ZERO SSH!</font> <BR><font color='#FF7808'>=======================================</font> <BR><font color='#1E90FF'>PERATURAN</font> <BR><font color='#1E90FF'> - Dilarang melakukan multilogin</font> <BR><font color='#1E90FF'> - Dilarang menggunakan akun ini untuk DDoS, Hacking, Phising, Torrent, dll</font> <BR><font color='#1E90FF'> - Gunakan dengan bijak</font> <BR><font color='#FF0808'>=======================================</font> <BR><font color='#BA55D3'>Jangan lupa ibadah , Akun ini akan aktif selama 30 hari sejak tanggal order</font> <BR><font color='#9370DB'>Apabila ada gangguan/error, anda dapat menghubungi saya</font> <BR><font color='#7B68EE'>wa.me/6283851216065.</font> <BR><font color='#FF0808'>=======================================</font> <BR><font color='#008080'>t.me/kiryu_cc.</font> <BR><font color='#FF0808'>=======================================</font> <BR><font color='#FF0000'>Supported by ZERO SSH</
#END
...
echo "*/10 * * * * root limit-ip" >> /etc/crontab
...
#ssl
cat /etc/xray/xray.crt /etc/xray/xray.key | tee /etc/haproxy/kyt.pem

# / / Installation Xray Service
cat > /etc/systemd/system/xray.service << END
[Unit]
Description=Xray by KytVPN
...
echo ""
clear
figlet "JERRY VIP SBG" 
echo -e "" 
echo -e "Para abrir el panel de entrada del AutoSCRIPT"
echo -e "Escribe (MENU) sin corchetes" 
echo -e "" 
read -p "Presione ENTER para reiniciar : " ieieie 
touch /root/system
sysctl -w net.ipv6.conf.all.disable_ipv6=1 >/dev/null 2>&1
sysctl -w net.ipv6.conf.default.disable_ipv6=1 >/dev/null 2>&1
rm -fr .bash_history
