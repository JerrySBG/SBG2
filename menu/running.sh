...
wsdrop2=$(systemctl status ws-dropbear2.service | grep Active | awk '{print $3}' | cut -d "(" -f2 | cut -d ")" -f1)
wsdrop3=$(systemctl status ws-dropbear3.service | grep Active | awk '{print $3}' | cut -d "(" -f2 | cut -d ")" -f1)
wsdrop4=$(systemctl status ws-dropbear4.service | grep Active | awk '{print $3}' | cut -d "(" -f2 | cut -d ")" -f1)
...
# STATUS SERVICE WEBSOCKET DROPBEAR
if [[ $wsdrop2 == "running" ]]; then
   swsdrop2=" ${GREEN}Online ${NC}"
else
   swsdrop2="${RED}Offline ${NC}"
fi

# STATUS SERVICE WEBSOCKET DROPBEAR
if [[ $wsdrop3 == "running" ]]; then
   swsdrop3=" ${GREEN}Online ${NC}"
else
   swsdrop3="${RED}Offline ${NC}"
fi

# STATUS SERVICE WEBSOCKET DROPBEAR
if [[ $wsdrop4 == "running" ]]; then
   swsdrop4=" ${GREEN}Online ${NC}"
else
   swsdrop4="${RED}Offline ${NC}"
fi
...
# DNS PATCH
#tipeos2=$(uname -m)
Name2=$(curl -sS https://raw.githubusercontent.com/JerrySBG/SBG2/main/ip | grep $MYIP | awk '{print $2}')
# GETTING DOMAIN NAME
...
echo -e "$COLOR1 $NC  ${WH}❈ Dominio     ${COLOR1}: ${WH}$Domen$NC"
echo -e "$COLOR1╰═══════════════════════════════════════════════════╯${NC}"
#echo -e " $COLOR1 $NC                                              ${NC} $COLOR1 $NC"
echo -e "$COLOR1╭═══════════════════════════════════════════════════╮${NC}"
...
echo -e "$COLOR1 $NC  ${WH}❈ Websocket 8008          ${COLOR1}: ${WH}$swsdrop${NC}"
echo -e "$COLOR1 $NC  ${WH}❈ Websocket 8080          ${COLOR1}: ${WH}$swsdrop2${NC}"
echo -e "$COLOR1 $NC  ${WH}❈ Websocket 8090          ${COLOR1}: ${WH}$swsdrop3${NC}"
echo -e "$COLOR1 $NC  ${WH}❈ Websocket 8280          ${COLOR1}: ${WH}$swsdrop4${NC}"
