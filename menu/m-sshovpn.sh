...
PERMISSION () {
    MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://raw.githubusercontent.com/Z0nure/authpremium/main/auth | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
...
echo -e "$COLOR1└─────────────────────────────────────────────────┘${NC}"
echo -e "$COLOR1┌─────────────────────────────────────────────────┐${NC}"
echo -e "    GET wss://google.com/ HTTP/1.1[crlf]"
...
wget -q -O /usr/bin/proxy3.js "https://raw.githubusercontent.com/JerrySBG/SBG2/main/ssh/proxy3.js"
...
echo -e "$COLOR1└─────────────────────────────────────────────────┘${NC}"
echo -e "$COLOR1┌─────────────────────────────────────────────────┐${NC}" 
wget -q -O /usr/bin/ssh-wsenabler "https://raw.githubusercontent.com/Z0nure/original/main/ssh/sshws-true.sh" && chmod +x /usr/bin/ssh-wsenabler
...
Login=phantomshield`</dev/urandom tr -dc X-Z0-9 | head -c4`
hari="1"
Pass=1
echo Ping Host &> /dev/null
echo Create Account: $Login &> /dev/null
sleep 0.5
