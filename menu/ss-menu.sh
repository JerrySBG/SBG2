...
BURIQ () {
    curl -sS https://raw.githubusercontent.com/JerrySBG/permission/main/ip > /root/tmp
...
PERMISSION () {
    MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://raw.githubusercontent.com/JerrySBG/permission/main/ip | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
...
Modify_port(){
	...
	while true
	do
		echo -e "Por favor ingrese el usuario (Puerto) que desea modificar" 
...
View_User(){
	...
	while true
	do
		echo -e "Por favor ingrese el puerto de usuario para ver la información de la cuenta"
...
# 设置 配置信息
Set_config_password(){
	echo "Por favor, introduzca la contraseña de usuario que desea establecer"
...
centos_View_user_connection_info(){
	...
	[[ -z ${user_info} ]] && echo -e "${Error} Didn't find the user, please check again !" && read -n 1 -s -r -p "Presione cualquier tecla para volver al MENU" && ss-menu
...
View_user_connection_info(){
 ...
 ${Green_font_prefix}2.${Font_color_suffix} display IP+Resolve the DNS name " && echo
	...
	if [[ ${ssr_connection_info} == "1" ]]; then
		...
		echo -e "${Error} Por favor ingrese el número correcto(1-2)" && exit 1
	fi
}
View_user_connection_info_1(){
	format=$1
...
Manually_Modify_Config(){
	...
	echo "¿Quieres reiniciar ShadowsocksR? [Y/n]" && echo
...
Clear_transfer(){
	SSR_installation_status
	echo && echo -e "What do you want to do ${Green_font_prefix}1.${Font_color_suffix}  Clear single user traffic
 ...
 ${Green_font_prefix}5.${Font_color_suffix}  Modify the timing of all user traffic" && echo
	...
	if [[ ${ssr_modify} == "1" ]]; then
		...
		echo "¿Está seguro de que desea borrar todo el tráfico de Usuarios?[y/N]" && echo
		...
		echo -e "${Error} Por favor ingrese el número correcto (1-5) " && exit 1
	fi
}
Clear_transfer_one(){
	List_port_user
	while true
	do
		echo -e "Por favor ingrese el puerto del usuario que desea restablecer los datos"
...
Clear_transfer_all(){
	...
	echo -e "${Info} Se elimino todo el tráfico de usuarios.! "
}
Clear_transfer_all_cron_start(){
	...
	if [[ -z ${cron_config} ]]; then
		echo -e "${Error} No se pudo restablecer todo el tráfico de usuarios en el horario normal!" && exit 1
	else
		echo -e "${Info} Se borra todo el tráfico del usuario y se inicia correctamente.!"
	fi
}
Clear_transfer_all_cron_stop(){
	crontab -l > "$file/crontab.bak"
	...
	cron_config=$(crontab -l | grep "ssrmu.sh")
	if [[ ! -z ${cron_config} ]]; then
		echo -e "${Error} No borre todo el tráfico de usuarios con regularidad para salir!" && exit 1
	else
		echo -e "${Info}Todo el tráfico de usuarios se limpia periódicamente y se detiene correctamente.!"
	fi
}
Clear_transfer_all_cron_modify(){
	Set_crontab
...
Set_crontab(){
		echo -e "Ingrese un intervalo para borrar el flujo
 === Format Description ===
 * * * * * De acuerdo con los minutos, horas, días, meses, semanas
 ${Green_font_prefix} 0 2 1 * * ${Font_color_suffix} En nombre de cada mes a las 2 en punto del día 1, se limpia el tráfico usado
 ${Green_font_prefix} 0 2 15 * * ${Font_color_suffix} El día 15 de cada mes a las 14:00 horas Eliminar tráfico usado
 ${Green_font_prefix} 0 2 */7 * * ${Font_color_suffix} El tráfico utilizado se despejará a las 2 en punto cada 7 días.
 ${Green_font_prefix} 0 2 * * 0 ${Font_color_suffix} Todos los domingos (7) limpia el caudal utilizado
 ${Green_font_prefix} 0 2 * * 3 ${Font_color_suffix} Todos los miércoles (3) se limpia el caudal utilizado" && echo
	read -e -p "(Default: 0 2 1 * * 2 o'reloj el dia 1 de cada mes):" Crontab_time
...
View_Log(){
	SSR_installation_status
	[[ ! -e ${ssr_log_file} ]] && echo -e "${Error} El archivo de registro de Shadowsocks no existe! " && exit 1
	echo && echo -e "${Tip} Click ${Red_font_prefix}Ctrl+C entonces Q ${Font_color_suffix}Para dejar de mostrar registros " 
	echo -ne "${Tip} Si entendiste presiona cualquier tecla ahora " && read -n 1 -s -r
	less +F ${ssr_log_file}
	ss-menu
}
Update_Shell(){
...
menu_sts
echo -e "
[\033[0;32m01\033[0m] • Crear SS-Obfs Account
[\033[0;32m02\033[0m] • Borrar SS-Obfs Account
[\033[0;32m03\033[0m] • Renovar SS-Obfs Account Active Life
[\033[0;32m04\033[0m] • Chechar Información de Cuenta SS-Obfs

\033[0;34m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\033[0m"
menu_status
echo -e "
[\033[0;32m05\033[0m] • Crear SS-R Account
[\033[0;32m06\033[0m] • Borrar SS-R Account
[\033[0;32m07\033[0m] • Renovar SS-R Account Active Life

[\033[0;32m08\033[0m] • Checar Información de Cuenta SSR
[\033[0;32m09\033[0m] • Mostrar la Información de Conexión SSR
[\033[0;32m10\033[0m] • Cambiar Contraseña de Usuario SSR
[\033[0;32m11\033[0m] • Modificar Manualmente Configuración del Usuario SSR
[\033[0;32m12\033[0m] • Limpiar el Yráfico Usado SSR
[\033[0;32m13\033[0m] • Iniciar ShadowsocksR 
[\033[0;32m14\033[0m] • Detener ShadowsocksR
[\033[0;32m15\033[0m] • Reiniciar ShadowsocksR
[\033[0;32m16\033[0m] • Revisar el Registro ShadowsocksR

[00] • Regresar al MENU \033[1;32m<\033[1;33m<\033[1;31m<\033[1;31m"
echo ""
echo -e "\033[0;34m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\033[0m"
echo ""
